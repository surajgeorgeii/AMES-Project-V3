...
function setupReviewButtons() {
    document.querySelectorAll('.review-btn').forEach(button => {
        button.addEventListener('click', (e) => {
            ...
            window.location.href = `/module-lead/modules/${moduleId}/review`;
        });
    });
}
...
async function loadModulesTable(page = 1, search = '', sort = currentSort, direction = currentDirection) {
    try {
        ...
        const response = await fetch(`/module-lead/modules/pending?page=${page}&search=${encodeURIComponent(search)}&sort=${sort}&direction=${direction}`);
...
async function handleReviewSubmit(e) {
    ...
    try {
        ...
        if (data.success) {
            ...
            setTimeout(() => {
                window.location.href = '/module-lead/modules';
            }, 1500);
        } else {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function loadModules(params = {}) {
        try {
            ...
            const response = await fetch(`/module-lead/modules/pending?${queryParams}`, {
